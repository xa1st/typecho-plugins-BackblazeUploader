 ...
 */
class BackblazeUploader_Plugin implements Typecho_Plugin_Interface
{
  /**
  ...
  public static function activate() {
    Typecho_Plugin::factory('Widget_Upload')->uploadHandle = ['BackblazeUploader_Plugin', 'uploadHandle'];
    Typecho_Plugin::factory('Widget_Upload')->modifyHandle = ['BackblazeUploader_Plugin', 'modifyHandle'];
    Typecho_Plugin::factory('Widget_Upload')->deleteHandle = ['BackblazeUploader_Plugin', 'deleteHandle'];
    Typecho_Plugin::factory('Widget_Upload')->attachmentHandle = ['BackblazeUploader_Plugin', 'attachmentHandle'];
  ...
  public static function config(Typecho_Widget_Helper_Form $form) {
    $keyId = new Typecho_Widget_Helper_Form_Element_Text(
      'keyId', 
      null, 
      '', 
      _t('应用密钥ID'), 
      _t('Backblaze B2的应用密钥ID')
    );
    $form->addInput($keyId->addRule('required', _t('必须填写应用密钥ID')));
    
    $applicationKey = new Typecho_Widget_Helper_Form_Element_Password(
      'applicationKey', 
      null, 
      '', 
      _t('应用密钥'), 
      _t('Backblaze B2的应用密钥')
    );
    $form->addInput($applicationKey->addRule('required', _t('必须填写应用密钥')));
    
    $bucketId = new Typecho_Widget_Helper_Form_Element_Text(
      'bucketId', 
      null, 
      '', 
      _t('存储桶ID'), 
      _t('Backblaze B2的存储桶ID')
    );
    $form->addInput($bucketId->addRule('required', _t('必须填写存储桶ID')));
    
    $bucketName = new Typecho_Widget_Helper_Form_Element_Text(
      'bucketName', 
      null, 
      '', 
      _t('存储桶名称'), 
      _t('Backblaze B2的存储桶名称')
    );
    ...
    $domain = new Typecho_Widget_Helper_Form_Element_Text(
      ...
      '', 
      _t('自定义域名'), 
      _t('如果您使用了自定义域名，请填写您的域名，例如：https://typecho.com，不包含尾部斜杠')
    );
    ...
    $path = new Typecho_Widget_Helper_Form_Element_Text(
      ...
      'typecho/', 
      _t('存储路径'), 
      _t('文件存储在存储桶中的路径前缀，以/结尾，例如：typecho/')
    );
    $form->addInput($path);
  }
  ...
  private static function uploadToBackblaze(string $file, string $target): bool {
    ...
    $sha1 = sha1_file($file);
    $contentType = Typecho_Common::mimeContentType($file);
    ...
    try {
      $response = $client->setData($fileContent)->setMethod(Typecho_Http_Client::METHOD_POST)->send($uploadAuth['uploadUrl']);
  ...
  private static function deleteFile(string $path): bool {
    ...
    // 删除文件
    $client = Typecho_Http_Client::get();
    $client->setHeader('Authorization', $auth['authorizationToken'])->setHeader('Content-Type', 'application/json');
    try {
      $response = $client->setData(json_encode(['fileName' => $fileName, 'fileId' => $fileId]))
  ...
  private static function getBackblazeAuth(string $keyId, string $applicationKey) {
    ...
    try {
      $response = $client->setMethod(Typecho_Http_Client::METHOD_GET)->send('https://api.backblazeb2.com/b2api/v2/b2_authorize_account');
      if ($response->getStatus() === 200) return json_decode($response->getBody(), true);
    } catch (Exception $e) {
      return false;
    }
    return false;
  ...
  private static function getUploadUrl(array $auth, string $bucketId) {
    ...
    $client->setHeader('Authorization', $auth['authorizationToken'])->setHeader('Content-Type', 'application/json');
    try {
      $response = $client->setData(json_encode(['bucketId' => $bucketId]))
                          ->setMethod(Typecho_Http_Client::METHOD_POST)
  ...
  private static function getFileId(array $auth, string $bucketName, string $fileName) {
    ...
    $client->setHeader('Authorization', $auth['authorizationToken'])->setHeader('Content-Type', 'application/json');
    try {
      $response = $client->setData(json_encode(['bucketName' => $bucketName, 'prefix' => $fileName, 'maxFileCount' => 1 ]))
                          ->setMethod(Typecho_Http_Client::METHOD_POST)
