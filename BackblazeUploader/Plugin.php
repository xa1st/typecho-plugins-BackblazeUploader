...
class BackblazeUploader_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function attachmentHandle(array $content)
    {
        ...
        if (!empty($options->domain)) {
            ...
            // 使用Backblaze默认域名
            $url = 'https://f002.backblazeb2.com/file/' . $options->bucketName . '/' . 
                   $options->path . $content['attachment']->path;
        }
    ...
    private static function uploadToBackblaze(string $file, string $target): bool
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $uploadAuth['uploadUrl']);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $fileContent);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Authorization: ' . $uploadAuth['authorizationToken'],
            'X-Bz-File-Name: ' . urlencode($target),
            'Content-Type: ' . $contentType,
            'X-Bz-Content-Sha1: ' . $sha1,
            'X-Bz-Info-Author: BackblazeUploader'
        ]);
    ...
    private static function deleteFile(string $path): bool
    {
        ...
        // 删除文件
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $auth['apiUrl'] . '/b2api/v2/b2_delete_file_version');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $auth['apiUrl'] . '/b2api/v2/b2_get_upload_url');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Authorization: ' . $auth['authorizationToken'],
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $auth['apiUrl'] . '/b2api/v2/b2_list_file_names');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Authorization: ' . $auth['authorizationToken'],
        ...
        if ($status === 200) {
            ...
            if (!empty($data['files']) && $data['files'][0]['fileName'] === $fileName) {
